...
import { isBinaryPath } from "./binary-extension";
...
export function generateFiles(
  ...
  for (const filePath of files) {
    ...
    if (tree.exists(computedPath)) {
      ...
      if (options.overwriteStrategy === OverwriteStrategy.ThrowIfExisting) {
        throw new Error(
    ...
    if (isBinaryPath(filePath)) {
      ...
      try {
        ...
        signale.error(`Error in ${filePath.replace(`${tree.root}/`, "")}:`);
        throw e;
      }
    }

    tree.write(computedPath, String(newContent));
    modifiedFiles.push(computedPath);
  }

  // formatFiles(modifiedFiles);
}
