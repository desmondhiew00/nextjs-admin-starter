...
export function set12Hours(date: Date, value: string, period: Period, setPeriod?: (p: Period) => void) {
  let hours = 0;
  if (value > "12") {
    hours = Number.parseInt(value, 10);
    setPeriod?.("PM");
  } else {
    hours = Number.parseInt(getValid12Hour(value), 10);
    setPeriod?.("AM");
  }
...
export function setDateByType(
  ...
  switch (type) {
    ...
    case "12hours": {
      ...
      return set12Hours(date, value, period, setPeriod);
    }
    default:
      return date;
  }
