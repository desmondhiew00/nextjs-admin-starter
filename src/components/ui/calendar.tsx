...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      classNames={{
        months: "flex relative space-x-2",
      ...
      components={{
        ...
        Nav: ({ className, children, ...props }) => {
          ...
          return (
            <nav className={cn("flex items-center", className)} style={props.style}>
              <Button
                variant="outline"
              ...
              <div className={cn("grid grid-cols-4 gap-y-2", className)} {...props}>
                {Array.from({ length: displayYears.to - displayYears.from + 1 }, (_, i) => {
                  const isBefore = startMonth
                    ? differenceInCalendarDays(new Date(displayYears.from + i, 12, 31), startMonth) < 0
                    : false;
