...
>(({ className, onChange, ...props }, ref) => {
  return (
    <RPNInput.default
      ...
      {...props}
      value={props.value ? (props.value?.startsWith("+") ? props.value : `+${props.value}`) : props.value}
    />
  );
});
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          size="lg"
          className={cn("flex gap-1 rounded-e-none px-3 h-10 border-r-0")}
          disabled={disabled}
        >
          <FlagComponent country={value} countryName={value} />
          {/* <span className="mx-1">{value ? `+${RPNInput.getCountryCallingCode(value)}` : "+00"}</span> */}
